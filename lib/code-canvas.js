 ...
 * @property {CodeCanvas} canvas - The canvas that created this context.
 ...
 * @property {CodeCanvas} owner
 * @property {ContextState} state - The state of the canvas.
 * @property {function} init - The function that is called when the operation is executed. It creates a new object.
 * */
...
class CodeCanvasGradient extends CodeCreateClass {
    ...
    init() {
        this.state.code.push(c => c.func(`const ${this.name}=${this.state.canvas.ctxName}.${this.func}`));
    }
}
...
class CodeCanvasPattern extends CodeCreateClass {
    ...
    setTransform(matrix) {
        this.state.code.push(g => g.func(`${this.name}.setTransform`, matrix));
    }

    init() {
        this.state.code.push(g => g.func(`const ${this.name}=${this.state.canvas.ctxName}.${this.func}`, ...this._create));
    }
}
...
class CodeCanvas {
    ...
    constructor(width, height) {
         ...
         * @type {null|string}
         */
        this.name=CodeCanvas.parent?'_c'+(++CodeCanvas.parent.idGen):null;

        /**
         * @type {string}
         */
        this.ctxName=CodeCanvas.parent?this.name+'_ctx':'ctx';
         ...
         * @type {string}
         */
        this.filename=CodeCanvas.parent?CodeCanvas.parent.filename+"$"+this.name:'';
    ...
    /**
     * @param {CodeGenerator} c
     */
    installParameterResolver(c) {
        c.resolveParameter=(p) =>{
            if(p instanceof CodeCreateClass) {
                if(p.state.canvas===this) { // local object
                    return p.name;
                } else {    // external canvas object
                    p.state.canvas.require(this, c);
                    return p.name;
                }
            } else if(p instanceof CodeCanvas) {
                if(p!==this) this.require(this, c);
                return p.name;
            }
    ...
    require(p, c) {
        if(this===CodeCanvas.parent) return;
        // console.log("Require ", this.name);
        // try {
        //     throw new Error()
        // }catch (e) {
        //     console.log(e);
        // }
        if(!this.created) {
            c.append(`const ${this.name}=new OffscreenCanvas(${this.width}, ${this.height});`);
            c.append(`const ${this.ctxName}=${this.name}.getContext('2d');`)
        }
        if(this.root.code.length>0) {
            // c.append('{');
            c.incIdent();
            this.generate(c);
            c.decIdent();
        ...
        return new Proxy(this.root, {
            get: (target, name)  => {
                ...
                return (...params) => {
                    ...
                    if(method.type==='state') {
                        target.code.push((c) => c.func(this.ctxName+'.'+name, ...params));
                    } else if(method.type==='draw') {
                        target.code.push((c) => c.func(this.ctxName+'.'+name, ...params));
                    } else if(method.type==='path') {
                        target.path.push((c) => c.func(this.ctxName+'.'+name, ...params));
                        ...
                        const vn=this.ctxName+'_v'+(++target.varGen);
            ...
            set: (target, name, value) => {
                if(canvasStatesSet.has(name)) {
                    ...
                    target.code.push((c) => c.set(this.ctxName+'.' + name, value));
                    return true;
                }
            }
        })
